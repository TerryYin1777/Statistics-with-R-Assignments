...
samp1 %>% summarise(xbar = mean(area), sam_med = median(area), sam_IQR = IQR(area), sam_sd = sd(area), sam_min = min(area), sam_max = max(area), sam_iqr1 = quantile(area,0.25), sam_iqr3 = quantile(area,0.75))
```
...
```{r sampling-dist}
# type your code for the Exercise here, and Run Document
sample_means50 = ames %>% rep_sample_n(size = 50, reps = 15000, replace = TRUE) %>% summarize(xbar = mean(area))
ggplot(sample_means50, aes(x = xbar))+
  geom_histogram(binwidth = 20)
```
...
# type your code for the Exercise here, and Run Document
sample_means_small = ames %>% rep_sample_n(size = 10, reps = 25, replace = TRUE) %>% summarize(xbar = mean(area))
ggplot(sample_means_small, aes(x = xbar))+
...
Price_mean = ames %>% sample_n(size = 50) %>% summarise(pm = mean(price))
```
...
# type your code for this Exercise here, and Run Document
sample15 = ames %>% sample_n(size = 15) %>% summarise(x_bar = mean(price))
```
...
# type your code for this Exercise here, and Run Document
sample_means15 = ames %>% rep_sample_n(size = 15, reps =2000, replace = TRUE) %>% summarize(x_bar = mean(price))
...
# type your code for this Exercise here, and Run Document
sample_means150 = ames %>% rep_sample_n(size = 150, reps =2000, replace = TRUE) %>% summarize(x_bar = mean(price))

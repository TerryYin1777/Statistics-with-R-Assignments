...
samp %>% select(area) %>% summarise(mean_sample = mean(area))
```
...
# type your code for the Exercise here, and Knit
ci = samp %>% summarise(lower = mean(area)-qnorm(0.975)*sd(area)/sqrt(n),
                        higher = mean(area)+qnorm(0.975)*sd(area)/sqrt(n))
...
```{r plot-99-perc-cis}
# type your code for the Exercise here, and Knit
ci99 = ci99 %>% mutate(cap = ifelse(lower<=params$mu & upper>=params$mu,"yes","no"))
ci99_data = data.frame(ci_id = c(1:50,1:50),ci_bounds = c(ci99$lower,ci99$upper),capture_mu = c(ci99$cap,ci99$cap))
ggplot(data = ci99_data, aes(x = ci_bounds, y = ci_id, group = ci_id, color = capture_mu))+
  geom_point(size = 2)+
  geom_line()+
  geom_vline(xintercept = params$mu, color = "green")

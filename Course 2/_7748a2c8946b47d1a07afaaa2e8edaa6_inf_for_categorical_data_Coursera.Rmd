...
us122 = us12 %>%  group_by(response) %>% summarize(count = n()) %>% mutate(total = sum(count)) %>% mutate(prop = round(count/total,2))
```
...
me = qnorm(0.975)*sqrt(us122$prop[1]*us122$prop[2]/us122$total[1])
```
...
cn = atheism %>% filter(nationality == "China")
hk = atheism %>% filter(nationality == "Hong Kong")
cn1 = cn %>% group_by(response) %>% summarize(count = n()) %>% mutate(total = sum(count), prop = round((count/total),3))
hk1 = hk %>% group_by(response) %>% summarize(count = n()) %>% mutate(total = sum(count), prop = round((count/total),3))
inference(y = response, data = cn, statistic = "proportion", method = "theoretical",type = "ci", success = "atheist")
...
inference(response,year, data = spain, statistic = "proportion", type = "ht", method = "theoretical", success = "atheist", null = 0, alternative = "twosided")
```
...
inference(response,year,us,type = "ht",statistic = "proportion",method = "theoretical",success = "atheist", null = 0, alternative = "twosided")
```

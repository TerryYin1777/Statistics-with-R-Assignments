...
sfo_feb_flights %>% summarise(mean_ad = mean(arr_delay),median_ad = median(arr_delay),n = n())
```
...
nycflights %>% mutate(dep_type = ifelse(dep_delay<= 5,"on time","delayed")) %>% group_by(origin) %>% summarise(ot_dep_rate = sum(dep_type == "on time")/n()) %>% arrange(desc(ot_dep_rate))
```
...
nycflights %>% select(arr_type,dep_type) %>% filter(dep_type == "delayed") %>% summarise(ot_arr_dd_rate=sum(arr_type == "on time")/n())
```
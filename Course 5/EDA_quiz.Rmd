...
countna = ames_train %>% is.na() %>% colSums()
countna
```
...
sd_neighbour = ames_train %>% group_by(Neighborhood) %>% summarise(sd = sd(price)) %>% arrange(desc(sd))
```
...
garage = ames_train %>% filter(is.na(Garage.Type)) %>% summarize(count = n())
garage
```
...
year = ames_train %>% mutate(p = ifelse(Year.Built>=1999,1,0)) %>% summarise(h = sum(p)/n())
summary(ames_train$price)
base = ames_train %>% filter(is.na(Bsmt.Cond)) %>% summarise(count = n())
```
...
ames_ga = ames_train %>% mutate(with.ga = ifelse(is.na(Garage.Type),"no","yes"))
inference(y = Lot.Area, x = with.ga, data = ames_ga, statistic = "mean", type = "ht", method = "theoretical",null = 0, alternative = "greater",  order = c("yes","no"))
```
...
area2000 = ames_train %>% filter(Lot.Area>2000) %>% summarise(count = n())
area2000
```
...
re = lm(log(price)~log(area), data = ames_train)
out = ames_train %>% mutate(re = abs(re$residuals)) %>% arrange(re)
tail(out,3)
...
single = ames_train %>% group_by(Neighborhood) %>% summarise(mean(Bldg.Type == "1Fam"))
single
```
...
ggplot(ames_train,aes(x = Bedroom.AbvGr,y = log(area)))+
  geom_point()
ggplot(ames_train,aes(x = Bedroom.AbvGr,y = log(area)))+
  geom_boxplot(aes(color = factor(Bedroom.AbvGr)))
```
...
finished = ames_train %>% filter(Bsmt.Unf.SF != "NA" & Bsmt.Unf.SF != 0) %>% summarise(mean = mean(Bsmt.Unf.SF))
```

...
```{r}
# type your code for the Exercise here, and Knit
bayes_inference(y = weight,data = nc,type = "ht",statistic = "mean",null = 5.5,alternative = "twosided")
```
...
smoker = nc %>% select(habit,weight) %>% filter(habit == "smoker")
nonsmoker = nc %>% select(habit,weight) %>% filter(habit == "nonsmoker")
IQR(smoker$weight)
...
```{r}
bayes_inference(y = lowbirthweight, data = nc, success = "low", statistic = "proportion", type = "ht", null = 0.075, alternative = "twosided",hypothesis_prior = c(H1 = 0.5,H2 = 0.5),beta_prior = c(75,925))
```

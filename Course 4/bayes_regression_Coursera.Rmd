...
ggplot(m_wage_iq,aes(x = .fitted,y = .resid))+
  geom_point(col = "steelblue")+
  geom_hline(yintercept = 0,linetype = "dashed")
ggplot(m_wage_iq,aes(sample = .resid))+
  stat_qq()
...
wage_red_nona = na.omit(wage_red)
bma_wage_red_nona = bas.lm(lwage~.-wage,data = wage_red_nona,prior = "BIC",modelprior = uniform())
summary(bma_wage_red_nona)
```
...
naive = bas.lm(lwage~.-wage,data = wage_no_na,prior = "ZS-null",modelprior = beta.binomial())
summary(naive)
```
...
par(mfrow = c(1,1))
# type your code for the Exercise here, and Knit
coef = coefficients(bma_wage_red_nona)
plot(coef, subset = c(8), ask=FALSE)
```
